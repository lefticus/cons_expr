...
namespace schmxpr {
template <typename T>
concept not_bool = !std::same_as<std::remove_cvref_t<T>, bool>;

static constexpr auto plus_equal = [](auto &lhs, const auto &rhs) -> auto &
  requires requires {
    requires not_bool<decltype(lhs)>;
    lhs += rhs;
  }
{ return lhs += rhs; };
...
{ return lhs < rhs; };
static constexpr auto greater_than = [](const auto &lhs,
                                        const auto &rhs) -> auto
  requires requires {
    requires not_bool<decltype(lhs)>;
    lhs > rhs;
...
static constexpr auto less_than_equal = [](const auto &lhs,
                                           const auto &rhs) -> auto
  requires requires {
    requires not_bool<decltype(lhs)>;
    lhs <= rhs;
...
{ return lhs <= rhs; };
static constexpr auto greater_than_equal = [](const auto &lhs,
                                              const auto &rhs) -> auto
  requires requires {
    requires not_bool<decltype(lhs)>;
    lhs >= rhs;
...
static constexpr bool logical_and(bool lhs, bool rhs) { return lhs && rhs; }
static constexpr bool logical_or(bool lhs, bool rhs) { return lhs || rhs; }
static constexpr bool logical_xor(bool lhs, bool rhs) { return lhs ^ rhs; };
...
constexpr std::pair<bool, int> parse_int(std::string_view input) {
  int parsed = 0;
  auto result =
...
constexpr Token next_token(std::string_view input) {
  ...
  constexpr auto make_token = [=](auto token_input, std::size_t size) {
    return Token{token_input.substr(0, size),
                 consume(token_input.substr(size), is_whitespace)};
  };
  ...
  if (input.starts_with('"')) {
    ...
    return make_token(input, std::distance(input.begin(), location));
  }

  const auto value = consume(input, [=](char character) {
    return !is_whitespace(character) && character != ')' && character != '(';
  });
  return make_token(input, std::distance(input.begin(), value.begin()));
}
...
struct Schmxpr {
  ...
  static constexpr SExpr for_each(Schmxpr &engine, Context &context,
                                  std::span<const SExpr> params) {
    if (params.size() != 2) {
      throw std::runtime_error(
          "Wrong number of parameters to for-each expression");
    }

    const auto &list = engine.eval_to<LiteralList>(context, params[1]);

    for (auto itr = list.items.begin(); itr != list.items.end(); ++itr) {
      engine.invoke_function(context, params[0],
                             std::span<const SExpr>{itr, std::next(itr)});
    }

    return SExpr{Atom{std::monostate{}}};
  }

  using Atom = std::variant<std::monostate, bool, int, double, Identifier,
                            std::string, evaluator, UserTypes...>;
    ...
    while (!token.parsed.empty()) {
      if (token.parsed == "(") {
        ...
        if (token.parsed.starts_with('"')) {
          ...
          retval.push_back(SExpr{Atom(
              std::string(token.parsed.substr(1, token.parsed.size() - 2)))});
        } else if (auto [did_parse, value] = parse_int(token.parsed);
                   did_parse) {
  ...
  consteval Schmxpr() : built_ins(make_built_ins()) {}

  constexpr SExpr invoke_function(Context &context, const SExpr &function,
                                  std::span<const SExpr> parameters) {
    SExpr resolved_function = eval(context, function);

    if (auto *lambda = std::get_if<Lambda>(&resolved_function.value);
        lambda != nullptr) {
      ...
      for (std::size_t index = 0; index < parameters.size(); ++index) {
        new_context.objects.emplace_back(lambda->parameter_names[index],
                                         eval(context, parameters[index]));
      }

      for (const auto &statement : lambda->statements) {
        eval(new_context, statement);
      }
  ...
  constexpr evaluator get_function(const SExpr &expr) {
    if (const auto *atom = std::get_if<Atom>(&expr.value); atom != nullptr) {
      if (const auto *func = std::get_if<evaluator>(atom); func != nullptr) {
        return *func;
      }
    }
  ...
  constexpr static evaluator make_evaluator(Ret (*)(Param...)) {
    return evaluator{[](Schmxpr &engine, Context &context,
                        std::span<const SExpr> params) -> SExpr {
      if (params.size() != sizeof...(Param)) {
        throw std::runtime_error("wrong param count");
      }

      auto impl = [&]<std::size_t... Idx>(std::index_sequence<Idx...>) {
        if constexpr (std::is_same_v<void, Ret>) {
          Func(engine.eval_to<std::remove_cvref_t<Param>>(context,
                                                          params[Idx])...);
          return SExpr{Atom{std::monostate{}}};
        } else {
  ...
  template <auto Func>
  constexpr static evaluator make_evaluator() {
  ...
  constexpr void add(std::string_view name, Value &&value) {
  ...
  constexpr SExpr eval_impl(Context &, const LiteralList &list) {
    return SExpr{list};
  }

  constexpr SExpr eval_impl(Context &, const Lambda &lambda) {
    return SExpr{lambda};
  }

  constexpr SExpr eval_impl(Context &context, const List &list) {
    if (!list.empty()) {
      return invoke_function(context, list[0],
                             {std::next(list.begin()), list.end()});
    } else {
      return SExpr{list};
    }
  }
  ...
  constexpr SExpr eval(Context &context, const SExpr &expr) {
    return std::visit(
        [this, &context](const auto &val) { return eval_impl(context, val); },
        expr.value);
  }

  template <typename Type>
  constexpr Type eval_to(Context &context, const SExpr &expr) {
    if constexpr (std::is_same_v<Type, LiteralList>) {
      ...
      if (const auto *atom = std::get_if<Atom>(&expr.value); atom != nullptr) {
        ...
        return eval_to<Type>(
            context, eval_impl(context, *std::get_if<List>(&expr.value)));
      }
    }
  }

  static constexpr SExpr list(Schmxpr &engine, Context &context,
                              std::span<const SExpr> params) {
    LiteralList result;

    for (const auto &param : params) {
      result.items.push_back(engine.eval(context, param));
    }

    return SExpr{result};
  }

  static constexpr SExpr lambda(Schmxpr &, Context &context,
                                std::span<const SExpr> params) {
    if (params.size() < 2) {
      throw std::runtime_error(
          "Wrong number of parameters to lambda expression");
    }
    ...
    for (const auto &name : std::get<List>(params[0].value)) {
      parameter_names.push_back(
          std::get<Identifier>(std::get<Atom>(name.value)).value);
    }

    return SExpr{Lambda{
        context, parameter_names, {std::next(params.begin()), params.end()}}};
  }

  static constexpr SExpr ifer(Schmxpr &engine, Context &context,
                              std::span<const SExpr> params) {
    if (params.size() != 3) {
      throw std::runtime_error("Wrong number of parameters to if expression");
    }
  ...
  template <auto Op>
  static constexpr SExpr binary_left_fold(Schmxpr &engine, Context &context,
                                          std::span<const SExpr> params) {
    auto sum = [&engine, &context,
                params]<typename Param>(Param first) -> SExpr {
      if constexpr (requires(Param p1, Param p2) { Op(p1, p2); }) {
        for (const auto &next : params.subspan(1)) {
          Op(first, engine.eval_to<Param>(context, next));
        }
    ...
    if (!params.empty()) {
      return std::visit(sum,
                        std::get<Atom>(engine.eval(context, params[0]).value));
    }
  ...
  template <auto Op>
  static constexpr SExpr binary_boolean_left_fold(
      Schmxpr &engine, Context &context, std::span<const SExpr> params) {
    auto sum = [&engine, &context,
                params]<typename Param>(Param first) -> SExpr {
      if constexpr (requires(Param p1, Param p2) { Op(p1, p2); }) {
